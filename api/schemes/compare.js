...
export default async function handler(req, res) {
  ...
  if (!scheme_ids || !Array.isArray(scheme_ids)) {
    return res.status(400).json({ error: 'scheme_ids array is required' });
  }
  ...
  try {
    ...
    for (const c of chunks) {
      ...
      const id = c.slug || title.toLowerCase().replace(/\s+/g, '-');
      if (!scheme_ids.includes(id)) continue;
    ...
    return res.status(200).json(Object.values(uniqueSchemes));
  } catch (error) {
    console.error('Error comparing schemes:', error);
    return res.status(500).json({ error: 'Failed to compare schemes' });
  }
}
